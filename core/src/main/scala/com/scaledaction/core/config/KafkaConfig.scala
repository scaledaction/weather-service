...
trait KafkaConfig extends AppConfig {
  ...
  //  val KafkaBrokers = withFallback[String](Try(kafka.getString("brokers")),
  //    "kafka.brokers") getOrElse "127.0.0.1:9092"

  //  val KafkaBrokers = withFallback[List[String]](Try(kafka.getList("brokers")),
  //    "kafka.brokers") getOrElse "127.0.0.1:9092"

  //  Failure(com.typesafe.config.ConfigException$WrongType: reference.conf: 24: brokers has type LIST rather than STRING)
  //Failure(com.typesafe.config.ConfigException$WrongType: reference.conf: 28: topic has type LIST rather than STRING)
  //
  //  val KafkaBrokers = kafka.getList("brokers")
  val KafkaBrokers = withFallback[ConfigList](Try(kafka.getList("brokers")),
    "kafka.brokers") getOrElse "127.0.0.1:9092"

  val KafkaTopic = withFallback[ConfigList](Try(kafka.getList("topic")),
